                  ...
                  <tr :class="expandedRow===r.sid?'row-expanded':''">
                    ...
                    <td style="font-weight:600">
                      <div style="display:flex;align-items:center;gap:.4rem">
                        {{ r.name }}
                        <span v-if="r.isRevised" class="revised-badge" :title="'学生已修改投递，最新版本：' + r.revisedAt">已更新</span>
                      </div>
                    </td>
                    <td style="color:var(--ink-2);font-size:.773rem;font-family:monospace">{{ r.sid }}</td>
                    <td style="color:var(--ink-2);font-size:.8rem">
                      <div>{{ r.date }}</div>
                      <div v-if="r.isRevised" style="font-size:.7rem;color:var(--amber)">更新 {{ r.revisedAt }}</div>
                    </td>
                    <td>
